#
# $ python new_demo_ref.py -m popo_models/mystic_frozen_darknet_yolov3_tiny_model.xml -i /opt/intel/openvino_2020.1.023/Intel-EdgeAI-Nanodegree/PeopleCounterApp/images/sample_dog.jpg
#
import logging
import threading
import os
...
def class_file_parser(add):
    labels_map = open(add).read().strip().split("\n")
    return labels_map
...
def parse_yolo_region(predictions, resized_image_shape, original_im_shape, params, threshold, is_proportional):
    ...
    labels_map = class_file_parser("coco.names")

    for row, col, n in np.ndindex(params.side, params.side, params.num):
        # Getting raw values for each detection bounding box
        bbox = predictions[0, n*bbox_size:(n+1)*bbox_size, row, col]
        #print(f"in {n*bbox_size} to {(n+1)*bbox_size}, row: {row}, col {col}")
        #print(f"bbox shape: {bbox.shape}")
        x, y, width, height, object_probability = bbox[:5]
        #print(f"x: {x}, y: {y}, w: {width}, h:{height}, prob: {object_probability}")
        class_probabilities = bbox[5:]
        #print(f"prob {object_probability}")
        ...
        print(f"class: {labels_map[class_id]}")
...
def put_highlighted_text(frame, message, position, font_face, font_scale, color, thickness):
    ...
    cv2.imshow("result_new", frame)
    cv2.imwrite("result_new.jpg", frame)
...
def main():
    ...
    net = IENetwork(args.model, os.path.splitext(args.model)[0] + ".bin")
    # ---------------------------------- 3. Load CPU extension for support specific layer ------------------------------
    if "CPU" in args.device:
        supported_layers = ie.query_network(net, "CPU")
        not_supported_layers = [lIENetwork for l in net.layers.keys() if l not in supported_layers]
